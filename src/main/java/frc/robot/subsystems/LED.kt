...
object LED : SubsystemBase() {
    ...
    fun twinkle() {
        for (i in 0..<ledBuffer.length) {
            if (Random.nextDouble() < 0.1) {
                brightnessLevels[i] = Random.nextInt(255)
            } else {
                brightnessLevels[i] = max(0, brightnessLevels[i] - 10)
            }

            val hue = Random.nextInt(180)
    ...
    fun robonautsLED() {
        if (robonautLEDTimer.advanceIfElapsed(0.1)) {
            for (i in 0..<ledBuffer.length) {
                val color = Random.nextInt(0, 100)
    ...
    fun laserbeam() {
        ...
        for (i in laserPositions.indices) {
            ...
            if (laserPositions[i]!! >= ledBuffer.length) {
                laserPositions[i] = -Random.nextInt(SPACING)
            }
        }
        leds.setData(ledBuffer)
    }
    ...
    fun createWave(
        startColor: Color,
        endColor: Color,
        wavelength: kotlin.Double,
        cycleDuration: kotlin.Double,
    ) {
        var phase =
