...
object Coral : SubsystemBase() {
    ...
    init {
        ...
        coralFeederMotor.configurator.apply(coralFeederConfiguration)
        coralScoreMotor.configurator.apply(coralFeederConfiguration)
        ...
        coralFeederMotor.configurator.apply(upMotorCurrentConfig)
        coralScoreMotor.configurator.apply(coralScoreCurrentConfig)
        ...
        coralScoreMotor.configurator.apply(coralScoreConfiguration)
        coralFeederMotor.configurator.apply(coralFeederConfiguration)
        ...
        if (!algaeIntaking && !coralScoring) {
            when {
                ...
                getCoralSensor() && !hasPiece -> {
                    ...
                    // detects it
                    coralState = CoralState.CORAL_SLOW
                    setHasPiece(true)
                }
                !getCoralSensor() && hasPiece -> {
                    coralState = CoralState.CORAL_HOLD
                }
                else -> {
                    coralState = CoralState.CORAL_SLOW
                }
